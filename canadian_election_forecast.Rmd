...
```
```{r}
# Let's try making a model using brms (Bayesian Regression Models) which means placing
# priors on the sigmas.
...
# Add 2015 vote to data
post_strat_census_data <-
  inner_join(post_strat_census_data, riding_results, by=c("constituencynumber"))
...
sum(riding_data$forecast_winner == riding_results$party)
```
